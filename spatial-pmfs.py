...
# alpha_values = np.asarray( [0.46, 0.47, 0.48, 0.49] )

if is_spatial == 1:
    N = N*N
for i in range(2):
    if i == 0:
        pmf_cust = np.random.geometric(0.1,N)
        pmf_cust = pmf_cust/(np.sum(pmf_cust))

        pmf_serv = np.random.geometric(0.1,N)
    ...
    with open('/storage/home/hcoda1/7/vsivaraman3/p-smaguluri3-0/spatial-matching-sim-vishal/sim_paths_spatial_' + file_name  + '.pkl', 'wb') as f:  
    # with open('./sim_paths_spatial_' + file_name  + '.pkl', 'wb') as f: 
