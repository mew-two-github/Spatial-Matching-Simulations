...
def run_max_weight_parallel( time_steps, N, pmf_cust, pmf_serv, W, alpha_values, gurobi_flag = 1 ):
    pool = multiprocessing.Pool(4)
    outputs = zip(*pool.map(run_max_weight, range(0, 10 * offset, offset)))
    Q_paths = []
    C_paths = []
    for res in outputs:
        Q_paths += res["QP"]
...
for N in Nvalues:
    ...
    time_steps_M = 100#000
    time_steps_B = 100#000
